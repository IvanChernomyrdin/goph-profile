...
func newTestHTTPServer(t *testing.T, db *sql.DB) *httptest.Server {
	...
	storage := services.NewMinIOStorage(minioClient, bucketName())
	publisher := services.NewRabbitPublisher(amqpCh, "avatars.exchange", "avatar.uploaded", "avatar.deleted")
	avatarService := services.NewAvatarService(repo, storage, publisher)

	healthService := &api.HealthService{}
	handler := api.NewHandler(healthService, avatarService)
